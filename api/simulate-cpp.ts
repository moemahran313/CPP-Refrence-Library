...
// Retry utility for transient model errors
async function generateWithRetry(client: GoogleGenAI, payload: any, maxRetries = 5, baseDelayMs = 2000) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.error(`[Gemini Sandbox Vercel Error] Attempt ${attempt} failed:`, errorMsg);
...
export default async function handler(req: any, res: any) {
  ...
  if (req.method === 'GET') {
    res.status(200).json({ status: "ok", service: "C++ Compiler AI Serverless Engine" });
    return;
  }
  ...
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      res.status(500).json({
        ...
        message: "The server requires your Gemini API Key. Please add 'GEMINI_API_KEY' in your Vercel Project Environment Variables."
      });
      return;
    }
...
Your final output response must consist ONLY of the markdown layout containing \`### [TERMINAL_OUTPUT]\` and \`### [COMPILER_LOGS]\`, exactly as shown in the examples.`;
    ...
    console.error("Vercel Compilation simulation error:", error);
    const errorMsg = error?.message || "";
    res.status(500).json({
      error: "Simulation execution failed",
      message: errorMsg || "An unexpected error occurred during execution."
    });
  }
}
